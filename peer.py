...
def setup():
	...
	peerPort = input("Please enter peer port no : ")
	settings = Settings(socket.gethostbyaddr(serverIP)[0], peerPort)
...
class Settings:
	def __init__(self, serverHost, peerPort):
		self.host = serverHost
		self.peerPort = int(peerPort)
		print("Set to connect to "+self.host+" from port "+str(self.peerPort)+"\nPlease restart peer to apply changes.")

def peerOperation(settings):
	...
	peerHandleSocket.bind((socket.gethostname(), settings.peerPort))
	
	try:
		print("Connecting to "+settings.host+" from "+socket.gethostname()+" on port "+str(settings.peerPort))
		serverHandleSocket.connect((settings.host, 12345))
		if login(serverHandleSocket, settings) == 1:
			while True:
				mode = input("Please enter \n  1 for performing computation or \n  2 for volunteering in a computation. ")
				if mode == '1':
...
def login(serverHandleSocket, settings):
	...
	loginDetails = {"category" : "login", "uname" : uname, "pwd" : pwd, "listeningPort" : settings.peerPort}
