...
def create(uname, pwd, addr):
    ...
    if uname in peerlist:
        ...
        peerlist[uname] = {"pwd": pwd, "ip": addr[0], "port": addr[1],
...
def getpeerlist(sock):
    ...
    if payload != False:
        for uname in payload:
            sendable_payload[uname] = {"ip": payload[uname]["ip"], "port": payload[uname]["port"],
                                       "last_seen": payload[uname]["last_seen"], "status": payload[uname]["status"]}
...
def process(msg, sock, addr):
    ...
    if content["cat"] == "login":
        ...
        if content["item"] == "peerlist":
            update_peerlist(content["uname"],
                            content["pwd"], content["status"])
        elif content["item"] == "modulelist":
            update_modulelist(content["uname"],
                              content["pwd"], content["list"])
